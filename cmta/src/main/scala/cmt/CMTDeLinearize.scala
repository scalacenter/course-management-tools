...
object CMTDeLinearize:
  ...
  def handleError(gitLogOutput: String): Vector[ExerciseNameAndSHA] =
    val errorMessage =
      toConsoleRed("""Couldn't obtain exercise info from linearized repository. Check your path to the latter""".stripMargin)
  ...
  def putBackToMain(mainRepo: File, linearizedRepo: File, exercisesAndSHAs: Vector[ExerciseNameAndSHA])
    ...
    val linearizedActiveExerciseFolder = linearizedRepo / config.linearizedRepoActiveExerciseFolder
    ...
    for (ExerciseNameAndSHA(exercise, sha) <- exercisesAndSHAs) {
      ...
      sbtio.copyDirectory(linearizedActiveExerciseFolder, mainRepoActiveExerciseFolder / exercise, preserveLastModified = true)
    }
