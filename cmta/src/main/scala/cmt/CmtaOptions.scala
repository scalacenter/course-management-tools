...
final case class Linearize(linearizeBaseFolder: Option[File] = None,
                           forceDeleteExistingDestinationFolder: Boolean = false) extends CmtaCommands
  ...
  checkConfig(config => config.command match
    ...
    case Linearize(Some(linBase), false) =>
      ...
      if linRepoBase.isDirectory then 
        failure(s"""Destination folder ${linBase.getName}/${config.mainRepo.getName} already exists:
