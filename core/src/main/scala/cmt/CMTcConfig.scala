...
class CMTcConfig(studentifiedRepo: File):
  ...
  val testCodeFolders = cmtSettings.getStringList("test-code-folders").asScala.toSet
  ...
  val previousExercise: Map[String, String] = exercises.tail.zip(exercises).to(Map)
