...
object Helpers:
  ...
  def isExerciseFolder(folder: File)(using config: CMTConfig): Boolean = {
  ...
  def createStudentifiedFolderSkeleton(stuBase: File, studentifiedRootFolder: File)(using config: CMTConfig, eofe: ExitOnFirstError) =
  ...
  def getExercises(mainRepo: File)(using config: CMTConfig, eofe: ExitOnFirstError): List[String] =
    sbtio
  ...
  def hideExercises(cleanedMainRepo: File, solutionsFolder: File, exercises: List[String])(using config: CMTConfig): Unit =
  ...
  def writeStudentifiedCMTConfig(configFile: File, exercises: Seq[String])(using config: CMTConfig): Unit =
    val cmtConfig =
  ...
  def withZipFile(activeExerciseFolder: File, solutionsFolder: File, exercise: String)(code: => Any): Unit =
    val archive = solutionsFolder / s"$exercise.zip"
    sbtio.unzip(archive, solutionsFolder)
    val files = 
      Helpers
        .fileList(solutionsFolder / s"$exercise")
    sbtio.copyDirectory(solutionsFolder / s"$exercise", activeExerciseFolder)
    sbtio.delete(solutionsFolder / exercise)
  end withZipFile